...
struct ScreenTimeComparisonCard: View {
    ...
    /// so the room is fixed: the percentage, two bars and two lines of copy.
    private static let reportHeight: CGFloat = 156

    @State private var now = Date()

    var body: some View {
        if screenTime.isAuthorized {
            DeviceActivityReport(.comparison, filter: .ransomDays(back: 1, until: now))
